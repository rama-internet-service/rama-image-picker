...
public class RecyclerViewManager {
  static FakeR fakeR;
    ...
    private void setItemDecoration(int columns) {
      ...
      /*itemOffsetDecoration = new GridSpacingItemDecoration(
        columns,
        context.getResources().getDimensionPixelSize(fakeR.getId("dimen", "ef_item_padding")),
        false
      );*/
      itemOffsetDecoration = new GridSpacingItemDecoration(
        columns,
...
*/
      ...
      return config.getLimit() == MAX_LIMIT
        ? String.format(context.getString(fakeR.getId("string", "ef_selected")), imageSize)
        : String.format(context.getString(fakeR.getId("string", "ef_selected_with_limit")), imageSize, config.getLimit());
    ...
    public boolean selectImage(boolean isSelected) {
        if (config.getMode() == MODE_MULTIPLE) {
            if (imageAdapter.getSelectedImages().size() >= config.getLimit() && !isSelected) {
              ...
              Toast.makeText(context, fakeR.getId("string", "ef_msg_limit_images"), Toast.LENGTH_SHORT).show();
