...
public class ImagePickerFragment extends Fragment implements ImagePickerView {
  private FakeR fakeR;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (isCameraOnly) {
          ...
          fakeR = new FakeR(getActivity().getApplicationContext());
          View result = localInflater.inflate(fakeR.getId("layout", "ef_fragment_image_picker"), container, false);
  ...
  private void setupView(View rootView) {
    progressBar = rootView.findViewById(fakeR.getId("id", "progress_bar"));

    emptyTextView = rootView.findViewById(fakeR.getId("id", "tv_empty_images"));
    recyclerView = rootView.findViewById(fakeR.getId("id", "recyclerView"));
    snackBarView = rootView.findViewById(fakeR.getId("id", "ef_snackbar"));
  }
    ...
    private void requestWriteExternalPermission() {
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            ...
            if (!preferences.isPermissionRequested(permission)) {
              ...
              snackBarView.show(fakeR.getId("string", "ef_msg_no_write_external_permission"), v -> openAppSettings());
        ...
        if (checkForRationale(permissions)) {
            ...
            if (!preferences.isPermissionRequested(permission)) {
                ...
                if (isCameraOnly) {
                  ...
                  getString(fakeR.getId("string", "ef_msg_no_camera_permission")), Toast.LENGTH_SHORT).show();

                    interactionListener.cancel();
                } else {
                   // snackBarView.show(R.string.ef_msg_no_camera_permission, v -> openAppSettings());
                  snackBarView.show(fakeR.getId("string", "ef_msg_no_camera_permission"), v -> openAppSettings());
